...
/*		this.bindDevice(launchpadID, launchpadHandler);
		this.bindDevice(\LPMiniMK3_MIDI_Out, launchpadHandler);
		this.bindDevice(\KEYBOARD, footControllerHandler);
		this.bindDevice(\to_SC, guitarHandler);*/
	...
	/*scanDevices {
    MIDIClient.sources.do { |src|
        var symbol = (src.device ++ "_" ++ src.name).replace(" ", "_").asSymbol;
        deviceUIDs[symbol] = src.uid;
    };
}*/
	//earlier version
...
/*	bindDevice { |symbol, handler|
		...
		if (srcID.notNil) {
			deviceHandlers[srcID] = handler;
			("🔗 Bound % to handler %".format(symbol, handler.class.name)).postln;
		} {
